...
const submitButton = buttons.find((node) => {
    ...
    return (
        compact === '注册' ||
        compact.includes('注册') ||
        lowerRaw.includes('sign up') ||
        lowerCompact.includes('signup') ||
        lowerRaw.includes('continue') ||
        lowerCompact.includes('continue') ||
        lowerRaw.includes('next') ||
        lowerCompact.includes('next')
    );
});
...
function isSubmitLabel(node) {
    const raw = (node.innerText || node.textContent || '').trim();
    const compact = raw.replace(/\\s+/g, '').toLowerCase();
    const lowerRaw = raw.toLowerCase();
    return (
        compact.includes('完成注册') ||
        compact.includes('创建账户') ||
        compact.includes('创建账号') ||
        compact.includes('注册') ||
        lowerRaw.includes('sign up') ||
        compact.includes('signup') ||
        lowerRaw.includes('create account') ||
...
const submitBtn = buttons.find(isSubmitLabel) || buttons.find((node) => node.type === 'submit') || null;
...
const submitBtn = buttons.find(isSubmitLabel) || buttons.find((node) => node.type === 'submit') || null;
if (!submitBtn) return 'no-submit-button:' + buttons.map((n) => (n.innerText || n.textContent || '').trim()).filter(Boolean).slice(0, 8).join('|');
submitBtn.focus();
submitBtn.click();
return 'submitted';
            """
